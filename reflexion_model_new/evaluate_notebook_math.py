...
Answer:"""
    ...
    def check_correct(self, pred, gt_str):
        ...
        elif "\\boxed{" in gt_str:
            try:
                start = gt_str.rfind("\\boxed{") + 7
                count = 1
                end = start
                while count > 0 and end < len(gt_str):
                    if gt_str[end] == '{': count += 1
                    elif gt_str[end] == '}': count -= 1
                    end += 1
                gold = self.extract_answer(gt_str[start:end-1])
            except:
    ...
    def run_scientific_test(self):
        ...
        dataset = full_dataset.train_test_split(test_size=0.1, seed=42)['test']
        
        # 为了快速演示，这里可以切片，全量测试请去掉切片
        # dataset = dataset.select(range(200)) 
        questions = dataset['problem']
        ground_truths = dataset['solution']
        total = len(questions)
        ...
        retrieved_batch = self.memory.batch_retrieve_active(query_embeddings, top_k=self.TOP_K, threshold=0.0)
        ...
        for i in range(total):
            ...
            if rag_ans is not None and rag_ans != base_ans:
                ...
                elif base_count < 3 and rag_count >= 2:
                    final_ans = rag_ans
